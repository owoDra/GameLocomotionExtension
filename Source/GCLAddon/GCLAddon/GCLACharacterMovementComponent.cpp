...
#include "Movement/CharacterMovementData.h"
#include "Movement/CharacterCustomMovement.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(GCLACharacterMovementComponent)


/////////////////////////////////////////////////////////////////////////
// FGCLACharacterNetworkMoveData

#pragma region FGCLACharacterNetworkMoveData

FGCLACharacterNetworkMoveData::FGCLACharacterNetworkMoveData()
{
...
void FGCLACharacterNetworkMoveData::ClientFillNetworkMoveData(const FSavedMove_Character& Move, ENetworkMoveType MoveType)
{
	Super::ClientFillNetworkMoveData(Move, MoveType);

	const auto& SavedMove{ static_cast<const FGCLASavedMove&>(Move) };
...
bool FGCLACharacterNetworkMoveData::Serialize(UCharacterMovementComponent& Movement, FArchive& Archive, UPackageMap* Map, const ENetworkMoveType MoveType) 
{
...
#pragma region FGCLACharacterNetworkMoveDataContainer

FGCLACharacterNetworkMoveDataContainer::FGCLACharacterNetworkMoveDataContainer()
{
...
#pragma region FGCLASavedMove

FGCLASavedMove::FGCLASavedMove()
{
	RotationMode	= TAG_Status_RotationMode_ViewDirection;
	Stance			= TAG_Status_Stance_Standing;
	Gait			= TAG_Status_Gait_Walking;
...
void FGCLASavedMove::SetMoveFor(ACharacter* Character, float NewDeltaTime, const FVector& NewAcceleration, FNetworkPredictionData_Client_Character& PredictionData)
{
	Super::SetMoveFor(Character, NewDeltaTime, NewAcceleration, PredictionData);

	const auto* Movement{ Cast<UGCLACharacterMovementComponent>(Character->GetCharacterMovement()) };
	if (IsValid(Movement))
	{
...
bool FGCLASavedMove::CanCombineWith(const FSavedMovePtr& NewMovePtr, ACharacter* Character, float MaxDelta) const
{
	const auto* NewMove{ static_cast<FGCLASavedMove*>(NewMovePtr.Get()) };
...
void FGCLASavedMove::CombineWith(const FSavedMove_Character* PreviousMove, ACharacter* Character, APlayerController* Player, const FVector& PreviousStartLocation)
{
	// Calling Super::CombineWith() forces the character's rotation to change to the rotation of the previous move.
	// This is undesirable because it will erase any rotation changes made in the ACharacter class. Therefore, do not change the rotation,
	// After calling Super::CombineWith(), they are restored.
...
void FGCLASavedMove::PrepMoveFor(ACharacter* Character)
{
	Super::PrepMoveFor(Character);

	auto* Movement{ Cast<UGCLACharacterMovementComponent>(Character->GetCharacterMovement()) };
	if (IsValid(Movement))
	{
...
#pragma region FGCLANetworkPredictionData

FGCLANetworkPredictionData::FGCLANetworkPredictionData(const UGCLACharacterMovementComponent& Movement)
	: Super(Movement)
{
}

FSavedMovePtr FGCLANetworkPredictionData::AllocateNewMove()
{
	return MakeShared<FGCLASavedMove>();
}
...
//////////////////////////////////////////////
// UGCLACharacterMovementComponent

const FName UGCLACharacterMovementComponent::NAME_ActorFeatureName("CharacterMovement");

UGCLACharacterMovementComponent::UGCLACharacterMovementComponent(const FObjectInitializer& ObjectInitializer)
...
#if WITH_EDITOR
bool UGCLACharacterMovementComponent::CanEditChange(const FProperty* Property) const
{
...
void UGCLACharacterMovementComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ThisClass, MovementData);
}

FNetworkPredictionData_Client* UGCLACharacterMovementComponent::GetPredictionData_Client() const
{
	if (ClientPredictionData == nullptr)
	{
		auto* MutableThis{ const_cast<UGCLACharacterMovementComponent*>(this) };

		MutableThis->ClientPredictionData = new FGCLANetworkPredictionData(*this);
	}
...
void UGCLACharacterMovementComponent::OnRegister()
{
	Super::OnRegister();

	// This component can only be added to classes derived from AGCLACharacter

	const auto* Character{ GetOwner<AGCLACharacter>() };
	ensureAlwaysMsgf((Character != nullptr), TEXT("[%s] on [%s] can only be added to GCLACharacter actors."), *GetNameSafe(StaticClass()), *GetNameSafe(GetOwner()));
...
void UGCLACharacterMovementComponent::BeginPlay()
{
...
void UGCLACharacterMovementComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
bool UGCLACharacterMovementComponent::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	check(Manager);
...
void UGCLACharacterMovementComponent::HandleChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState)
{
...
void UGCLACharacterMovementComponent::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
...
void UGCLACharacterMovementComponent::CheckDefaultInitialization()
{
...
void UGCLACharacterMovementComponent::OnRep_MovementData()
{
	CheckDefaultInitialization();
}

void UGCLACharacterMovementComponent::ApplyMovementData()
{
	check(MovementData);

	auto* GCLACharacter{ GetCharacterChecked<AGCLACharacter>() };

	UpdateLocomotionConfigs();

	SetReplicatedViewRotation(GCLACharacter->GetViewRotationSuperClass());
...
void UGCLACharacterMovementComponent::SetMovementData(const UCharacterMovementData* NewMovementData)
{
	if (GetOwner()->HasAuthority())
	{
		if (NewMovementData != MovementData)
		{
...
void UGCLACharacterMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
void UGCLACharacterMovementComponent::UpdateUsingAbsoluteRotation() const
{
...
void UGCLACharacterMovementComponent::UpdateVisibilityBasedAnimTickOption() const
{
...
void UGCLACharacterMovementComponent::UpdateMovementBase()
{
	const auto& BasedMovement{ CharacterOwner->GetBasedMovement() };
...
void UGCLACharacterMovementComponent::UpdateAnimInstanceMovement()
{
...
void UGCLACharacterMovementComponent::SetMovementMode(EMovementMode NewMovementMode, uint8 NewCustomMode)
{
...
void UGCLACharacterMovementComponent::SetLocomotionMode(const FGameplayTag& NewLocomotionMode)
{
...
void UGCLACharacterMovementComponent::UpdateLocomotionConfigs()
{
...
void UGCLACharacterMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	Super::OnMovementModeChanged(PreviousMovementMode, PreviousCustomMode);

	bCrouchMaintainsBaseLocation = true;

	// If the current MovementMode is Walking or NavWalking
	...
	// If the current MovementMode is Falling
	...
	// If the current MovementMode is Swimming
	...
	// If the current MovementMode is Custom
	...
	else if (MovementMode == MOVE_Custom)
	{
		...
		{
			if (auto& CustomMovement{ MovementData->CustomMovements[CustomMovementIndex] })
			{
...
void UGCLACharacterMovementComponent::SetDesiredRotationMode(FGameplayTag NewDesiredRotationMode)
{
...
void UGCLACharacterMovementComponent::Server_SetDesiredRotationMode_Implementation(FGameplayTag NewDesiredRotationMode)
{
...
void UGCLACharacterMovementComponent::SetRotationMode(FGameplayTag NewRotationMode)
{
...
void UGCLACharacterMovementComponent::SetDesiredStance(FGameplayTag NewDesiredStance)
{
...
void UGCLACharacterMovementComponent::Server_SetDesiredStance_Implementation(FGameplayTag NewDesiredStance)
{
...
void UGCLACharacterMovementComponent::SetStance(FGameplayTag NewStance)
{
...
bool UGCLACharacterMovementComponent::CanCrouchInCurrentState() const
{
	if (!CanEverCrouch())
	{
		return false;
...
void UGCLACharacterMovementComponent::SetDesiredGait(FGameplayTag NewDesiredGait)
{
...
void UGCLACharacterMovementComponent::Server_SetDesiredGait_Implementation(FGameplayTag NewDesiredGait)
{
...
void UGCLACharacterMovementComponent::CalcVelocity(float DeltaTime, float Friction, bool bFluid, float BrakingDeceleration)
{
...
void UGCLACharacterMovementComponent::SetGait(FGameplayTag NewGait)
{
	if (Gait != NewGait)
	{
		Gait = NewGait;
...
void UGCLACharacterMovementComponent::RefreshGaitConfigs()
{
...
void UGCLACharacterMovementComponent::RefreshGaitConfigs(const FCharacterGaitConfigs& GaitConfigs)
{
...
bool UGCLACharacterMovementComponent::TryGetMovementBaseRotationSpeed(const FBasedMovementInfo& BasedMovement, FRotator& RotationSpeed)
{
...
void UGCLACharacterMovementComponent::SetLocomotionAction(const FGameplayTag& NewLocomotionAction)
{
...
FVector UGCLACharacterMovementComponent::ConsumeInputVector()
{
...
void UGCLACharacterMovementComponent::SetInputDirection(FVector NewInputDirection)
{
...
void UGCLACharacterMovementComponent::UpdateInput(float DeltaTime)
{
	if (CharacterOwner->GetLocalRole() >= ROLE_AutonomousProxy)
	{
	...
	{
		LocomotionState.InputYawAngle = UE_REAL_TO_FLOAT(UMovementMathLibrary::DirectionToAngleXY(InputDirection));
	}
}
...
void UGCLACharacterMovementComponent::UpdateView(float DeltaTime)
{
...
void UGCLACharacterMovementComponent::CorrectViewNetworkSmoothing(const FRotator& NewViewRotation)
{
...
void UGCLACharacterMovementComponent::UpdateViewNetworkSmoothing(float DeltaTime)
{
	auto& NetworkSmoothing{ ViewState.NetworkSmoothing };
	...
	const auto InterpolationAmount{ UMovementMathLibrary::Clamp01(1.0f - (NetworkSmoothing.ServerTime - NetworkSmoothing.ClientTime) / NetworkSmoothing.Duration) };

	if (!FAnimWeight::IsFullWeight(InterpolationAmount))
	{
		NetworkSmoothing.Rotation = UMovementMathLibrary::LerpRotator(NetworkSmoothing.InitialRotation, ReplicatedViewRotation, InterpolationAmount);
...
void UGCLACharacterMovementComponent::SetReplicatedViewRotation(const FRotator& NewViewRotation)
{
...
void UGCLACharacterMovementComponent::OnReplicated_ReplicatedViewRotation()
{
	CorrectViewNetworkSmoothing(ReplicatedViewRotation);
}

void UGCLACharacterMovementComponent::Server_SetReplicatedViewRotation_Implementation(const FRotator& NewViewRotation)
{
...
void UGCLACharacterMovementComponent::UpdateCharacterStateBeforeMovement(float DeltaSeconds)
{
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UGCLACharacterMovementComponent::UpdateCharacterStateBeforeMovement()"), STAT_UGCLACharaterMovementComponent_UpdateCharacterStateBeforeMovement, STATGROUP_GCLAMovement);
...
void UGCLACharacterMovementComponent::UpdateCharacterStateAfterMovement(float DeltaSeconds)
{
	DECLARE_SCOPE_CYCLE_COUNTER(TEXT("UGCLACharacterMovementComponent::UpdateCharacterStateAfterMovement()"), STAT_UGCLACharaterMovementComponent_UpdateCharacterStateAfterMovement, STATGROUP_GCLAMovement);
...
void UGCLACharacterMovementComponent::ComputeFloorDist(
...
bool UGCLACharacterMovementComponent::CanAttemptJump() const
{
...
void UGCLACharacterMovementComponent::ControlledCharacterMove(const FVector& InputVector, float DeltaTime)
{
...
void UGCLACharacterMovementComponent::PhysWalking(float DeltaTime, int32 Iterations)
{
...
void UGCLACharacterMovementComponent::PhysCustom(float DeltaTime, int32 Iterations)
{
...
void UGCLACharacterMovementComponent::PerformMovement(float DeltaTime)
{
...
void UGCLACharacterMovementComponent::SmoothClientPosition(float DeltaTime)
{
...
void UGCLACharacterMovementComponent::MoveAutonomous(float ClientTimeStamp, float DeltaTime, uint8 CompressedFlags, const FVector& NewAcceleration)
{
	const auto* MoveData{ static_cast<FGCLACharacterNetworkMoveData*>(GetCurrentNetworkMoveData()) };
	if (MoveData != nullptr)
	{
...
bool UGCLACharacterMovementComponent::TryConsumePrePenetrationAdjustmentVelocity(FVector& OutVelocity)
{
...
void UGCLACharacterMovementComponent::SavePenetrationAdjustment(const FHitResult& Hit)
{
...
void UGCLACharacterMovementComponent::ApplyPendingPenetrationAdjustment()
{
...
void UGCLACharacterMovementComponent::UpdateLocomotionEarly()
{
	if (MovementBase.bHasRelativeRotation)
	{
...
void UGCLACharacterMovementComponent::UpdateLocomotion(float DeltaTime)
{
	...
	{
		LocomotionState.VelocityYawAngle = UE_REAL_TO_FLOAT(UMovementMathLibrary::DirectionToAngleXY(LocomotionState.Velocity));
	}
	...
	if (MovementData->bRotateTowardsDesiredVelocityInVelocityDirectionRotationMode && CharacterOwner->GetLocalRole() >= ROLE_AutonomousProxy)
	{
		...
		SetDesiredVelocityYawAngle(TryConsumePrePenetrationAdjustmentVelocity(DesiredVelocity) &&
			...
			? UE_REAL_TO_FLOAT(UMovementMathLibrary::DirectionToAngleXY(DesiredVelocity))
...
void UGCLACharacterMovementComponent::UpdateLocomotionLate(float DeltaTime)
{
...
void UGCLACharacterMovementComponent::UpdateLocomotionLocationAndRotation()
{
...
void UGCLACharacterMovementComponent::SetDesiredVelocityYawAngle(float NewDesiredVelocityYawAngle)
{
...
void UGCLACharacterMovementComponent::UpdateGroundedRotation(float DeltaTime)
{
	...
	if (!LocomotionState.bMoving)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if (ViewRelativeYawAngle > 180.0f - UMovementMathLibrary::CounterClockwiseRotationAngleThreshold)
					{
...
void UGCLACharacterMovementComponent::UpdateInAirRotation(float DeltaTime)
{
...
float UGCLACharacterMovementComponent::CalculateRotationInterpolationSpeed() const
{
	static constexpr auto MaxInterpolationSpeedMultiplier{ 3.0f };
	static constexpr auto ReferenceViewYawSpeed{ 300.0f };

	return RotationInterpSpeed * UMovementMathLibrary::LerpClamped(1.0f, MaxInterpolationSpeedMultiplier, ViewState.YawSpeed / ReferenceViewYawSpeed);
}

void UGCLACharacterMovementComponent::UpdateRotation(float TargetYawAngle, float DeltaTime, float RotationInterpolationSpeed)
{
	UpdateTargetYawAngle(TargetYawAngle);

	auto NewRotation{ CharacterOwner->GetActorRotation() };

	NewRotation.Yaw = UMovementMathLibrary::ExponentialDecayAngle(UE_REAL_TO_FLOAT(FRotator::NormalizeAxis(NewRotation.Yaw)), TargetYawAngle, DeltaTime, RotationInterpolationSpeed);
...
void UGCLACharacterMovementComponent::UpdateRotationExtraSmooth(float TargetYawAngle, float DeltaTime, float RotationInterpolationSpeed, float TargetYawAngleRotationSpeed)
{
	LocomotionState.TargetYawAngle = TargetYawAngle;

	UpdateViewRelativeTargetYawAngle();

	LocomotionState.SmoothTargetYawAngle = UMovementMathLibrary::InterpolateAngleConstant(LocomotionState.SmoothTargetYawAngle, TargetYawAngle, DeltaTime, TargetYawAngleRotationSpeed);

	auto NewRotation{ CharacterOwner->GetActorRotation() };

	NewRotation.Yaw = UMovementMathLibrary::ExponentialDecayAngle(UE_REAL_TO_FLOAT(FRotator::NormalizeAxis(NewRotation.Yaw)), LocomotionState.SmoothTargetYawAngle, DeltaTime, RotationInterpolationSpeed);
...
void UGCLACharacterMovementComponent::UpdateRotationInstant(float TargetYawAngle, ETeleportType Teleport)
{
	UpdateTargetYawAngle(TargetYawAngle);
...
void UGCLACharacterMovementComponent::UpdateTargetYawAngleUsingLocomotionRotation()
{
	UpdateTargetYawAngle(UE_REAL_TO_FLOAT(LocomotionState.Rotation.Yaw));
}

void UGCLACharacterMovementComponent::UpdateTargetYawAngle(float TargetYawAngle)
{
	LocomotionState.TargetYawAngle = TargetYawAngle;
...
void UGCLACharacterMovementComponent::UpdateViewRelativeTargetYawAngle()
{
...
void UGCLACharacterMovementComponent::ApplyRotationYawSpeed(float DeltaTime)
{
	const auto DeltaYawAngle{ CharacterOwner->GetMesh()->GetAnimInstance()->GetCurveValue(UCharacterAnimCurveNames::RotationYawSpeedCurveName()) * DeltaTime };
...
UGCLACharacterMovementComponent* UGCLACharacterMovementComponent::FindGCLACharacterMovementComponent(const ACharacter* Character)
{
	return (Character ? Character->FindComponentByClass<UGCLACharacterMovementComponent>() : nullptr);
}
